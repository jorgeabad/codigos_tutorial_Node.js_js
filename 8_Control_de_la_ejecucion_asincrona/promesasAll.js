...
function mostrarRespuesta(resuelta, rechazo) {
  if (resuelta) {
    console.log("| RESPUESTA a " +resuelta.url +" devuelve título:" + resuelta.datos.title);
    listado = `${listado} \n -${resuelta.datos.title}/${resuelta.datos.release_date}`;
  } else {
    console.log("| RESPUESTA a " + rechazo.url + " devuelve error: " + rechazo.error);
  }
...
for (let id = 1; id < 8; id++) {
  promesas.push(requestPromise(id));
}

Promise.all(promesas)
  .then(results => {
    results.map(objetoResolve => mostrarRespuesta(objetoResolve, undefined));
    console.log(listado);
